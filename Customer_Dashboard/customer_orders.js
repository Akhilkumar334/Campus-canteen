...
// =====================================
// CUSTOMER ORDERS — FINAL FIXED VERSION
// =====================================
...
// Global orders cache for view/reorder
let globalOrders = [];
...
// -------------------------------
// Fetch customer orders from GLOBAL ordersList
function gatherCustomerOrders() {
  ...
  allOrders.forEach(o => {
    if (!o) return;
    if (
  String(o.customer) === String(currentCustomer) ||
  (o.customer === "Guest" && currentCustomer === "Customer")
) 
 {
      result.push({
  ...
  // newest first
  result.sort((a, b) => {
    const ta = a.order.time || "";
    const tb = b.order.time || "";
    return ta < tb ? 1 : -1;
  });
...
function getVendorNameByEmail(email) {
  ...
  const homecooks = JSON.parse(localStorage.getItem("homecooksList")) || [];
  const hc = homecooks.find(h => h.email === email);
  if (hc) return hc.fullName || hc.kitchenName || "HomeCook";
...
// -------------------------------
// Render one order card
function renderOrderCard(vendorEmail, order, index) {
  const card = document.createElement("div");
  card.className = "order-card";
  ...
  card.innerHTML = `
    <div class="order-left">
      <div class="order-header">
        ...
        <div class="order-meta">${escapeHtml(niceTime(order.time))}</div>
      </div>
      <div class="order-meta"><strong>Vendor:</strong> ${escapeHtml(vendorName)}</div>
      <ul class="items-list">
        ${(order.items || []).slice(0, 5).map(i => `<li>${escapeHtml(i)}</li>`).join("")}
        ${(order.items && order.items.length > 5)
          ? `<li>and ${order.items.length - 5} more...</li>`
          : ""}
      </ul>
    </div>
...
function renderOrders() {
  ...
  if (!found.length) {
    ordersListEl.innerHTML =
      `<div class="order-card"><div style="color:#556;padding:12px;">You have no orders yet.</div></div>`;
    return;
  }
...
function viewDetails(index) {
  ...
  modalContent.innerHTML = `
    ...
    <p><strong>Vendor:</strong> ${escapeHtml(vendorName)}</p>
    <p><strong>Time:</strong> ${escapeHtml(niceTime(o.time))}</p>
    <p><strong>Status:</strong> <span class="status ${escapeHtml((o.status || "").toLowerCase())}">
...
function reorder(index) {
  ...
  const items = (order.items || []).map(s => {
    ...
    if (parts.length === 2) {
      return {
        name: parts[1].trim(),
        qty: Number(parts[0].trim()) || 1
      };
    }
    return { name: s, qty: 1 };
  });

  localStorage.setItem("reorder_cart", JSON.stringify({ vendorType, vendorEmail, items }));
  localStorage.setItem("selectedVendorType", vendorType);
  localStorage.setItem("selectedVendorEmail", vendorEmail);
